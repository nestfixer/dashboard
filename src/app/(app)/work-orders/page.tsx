...
export default async function WorkOrdersPage({
  ...
  const [workOrders, users] = await Promise.all([
    prisma.workOrder.findMany({
      where,
      include: {
        customer: { select: { name: true, isPropertyManagement: true } },
                    ...
                    <td className="px-4 py-3">
                      {wo.customer ? (
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="text-slate-500">{wo.customer.name}</span>
                          {wo.customer.isPropertyManagement && (
                            <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-purple-500/15 text-purple-500">
