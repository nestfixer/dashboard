      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        {customers.length === 0 ? (
          <div className="py-16 text-center flex flex-col items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-gray-700">{search ? "No customers match your search" : "No customers yet"}</p>
            ...
            {!search && (
              <Link href="/customers/new" className="mt-1 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition-colors">
                + New Customer
              </Link>
            )}
          </div>
              ...
              {customers.map((c) => (
                <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3">
                    <Link href={`/customers/${c.id}`} className="font-medium text-gray-900 hover:text-indigo-600 transition-colors">
                      {c.name}
                    </Link>
                    {c.address && (
