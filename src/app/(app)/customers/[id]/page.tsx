    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href="/customers" className="text-sm text-muted hover:text-white transition-colors">← Customers</Link>
      ...
      {confirmDelete && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={() => setConfirmDelete(false)} className="px-3 py-1.5 text-sm text-muted hover:bg-white/5 rounded-lg transition-colors">Cancel</button>
      ...
      <div className="bg-card rounded-xl border border-border p-6">
        {editing ? (
          <form onSubmit={handleSave} className="space-y-4">
            <h3 className="text-sm font-semibold text-white mb-4">Edit Customer</h3>
            <div>
              <label className="block text-xs font-medium text-muted mb-1">Name *</label>
              <input value={name} onChange={(e) => setName(e.target.value)} required className="w-full rounded-lg border border-border bg-card text-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-blue" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-medium text-muted mb-1">Phone</label>
                <input value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full rounded-lg border border-border bg-card text-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-blue" />
              </div>
              <div>
                <label className="block text-xs font-medium text-muted mb-1">Email</label>
                <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" className="w-full rounded-lg border border-border bg-card text-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-blue" />
              </div>
            </div>
            <div>
              <label className="block text-xs font-medium text-muted mb-1">Address</label>
              <input value={address} onChange={(e) => setAddress(e.target.value)} className="w-full rounded-lg border border-border bg-card text-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-blue" />
            </div>
            <div>
              <label className="block text-xs font-medium text-muted mb-1">Notes</label>
              <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} className="w-full rounded-lg border border-border bg-card text-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-blue resize-none" />
            </div>
            <div className="flex gap-3 pt-2">
              ...
              <button type="button" onClick={() => setEditing(false)} className="px-4 py-2 text-sm text-muted hover:bg-white/10 rounded-lg transition-colors">Cancel</button>
            </div>
          </form>
        ) : (
          <div className="grid grid-cols-2 gap-x-8 gap-y-4 text-sm">
            <div>
              ...
              <p className="text-white">{customer.phone || <span className="text-muted">—</span>}</p>
            </div>
            <div>
              <p className="text-xs font-medium text-muted mb-0.5">Email</p>
              <p className="text-white">{customer.email || <span className="text-muted">—</span>}</p>
            </div>
            <div>
              <p className="text-xs font-medium text-muted mb-0.5">Address</p>
              <p className="text-white">{customer.address || <span className="text-muted">—</span>}</p>
            </div>
            <div>
              <p className="text-xs font-medium text-muted mb-0.5">Customer since</p>
              <p className="text-white">{format(new Date(customer.createdAt), "MMMM d, yyyy")}</p>
            </div>
            {customer.notes && (
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-white">Work Orders ({customer.workOrders.length})</h3>
                ...
                {customer.workOrders.map((wo) => (
                  <tr key={wo.id} className="hover:bg-white/5 transition-colors">
                    <td className="px-4 py-3">
                      <Link href={`/work-orders/${wo.id}`} className="font-medium text-white hover:text-accent-blue transition-colors">{wo.title}</Link>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_STYLES[wo.status] ?? "bg-white/10 text-gray-400"}`}>{wo.status}</span>
                    </td>
